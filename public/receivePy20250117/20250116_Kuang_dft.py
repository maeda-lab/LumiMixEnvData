...
if image1 is None or image2 is None:
    ...
    def calculate_spectra(image):
        ...
        return power_spectrum, phase_spectrum, freq_y, freq_x

    power1, phase1, freq_y1, freq_x1 = calculate_spectra(image1)
    power2, phase2, freq_y2, freq_x2 = calculate_spectra(image2)
    ...
    # axes[1, 0].imshow(power1, cmap="gray", vmin=0, vmax=255)
    axes[1, 0].contourf(power1, cmap="gray", vmin=0, vmax=255)
    axes[1, 0].set_title("Image 1 (Power Spectrum)")
    axes[1, 0].axis("off")

    axes[3, 0].imshow(phase1, cmap="gray")
    axes[3, 0].set_title("Image 1 (Phase Spectrum)")
    axes[3, 0].axis("off")
    ...
    # axes[1, 1].imshow(power2, cmap="gray", vmin=0, vmax=255)
    axes[1, 1].contourf(power2, cmap="gray", vmin=0, vmax=255)
    axes[1, 1].set_title("Image 2 (Power Spectrum)")
    axes[1, 1].axis("off")

    axes[3, 1].imshow(phase2, cmap="gray")
    axes[3, 1].set_title("Image 2 (Phase Spectrum)")
    axes[3, 1].axis("off")

    # 差分
    axes[0, 2].axis("off")  # 差分の上段は空白
    axes[1, 2].imshow(power_diff, cmap="seismic", vmin=-255, vmax=255)
    ...
    # axes[3, 0].axis("off")
    s = np.shape(power1)
    axes[2, 0].plot(freq_x1, power1[int(s[0]/8)],'o', markersize=2, label='height 1/8')
    axes[2, 0].plot(freq_x1, power1[int(s[0]/4)],'o', markersize=2, label='height 1/4')
    axes[2, 0].plot(freq_x1, power1[int(s[0]/2)],'o', markersize=2, label='height 1/2')
    axes[2, 0].set_ylim(0, 350)
    axes[2, 0].set_title("Power Spectrum")
    axes[2, 0].set_xlabel("Normalized Spatial Frequency")
    axes[2, 0].set_xlim(-0.5, 0.5)
    # axes[2, 0].set_xlim(0,int(s[1]/2))
    axes[2, 0].legend()
    
    # axes[3, 1].axis("off")
    s = np.shape(power2)
    axes[2, 1].plot(freq_x2, power2[int(s[0]/8)], 'o', markersize=2, label='height 1/8')
    axes[2, 1].plot(freq_x2, power2[int(s[0]/4)],'o', markersize=2,  label='height 1/4')
    axes[2, 1].plot(freq_x2, power2[int(s[0]/2)], 'o', markersize=2, label='height 1/2')
    axes[2, 1].set_ylim(0,350)
    axes[2, 1].set_title("Power Spectrum")
    axes[2, 1].set_xlabel("Normalized Spatial Frequency")
    # axes[2, 1].set_xlim(0,int(s[1]/2))
    axes[2, 1].legend()

    # axes[3, 1].axis("off")
    s = np.shape(power2)
    axes[2, 2].plot(freq_x1,power1[int(s[0]/2)], 'o', markersize=2, label='Dots height 1/2')
    axes[2, 2].plot(freq_x2,power2[int(s[0]/2)], 'o', markersize=2, label='Natural height 1/2')
    axes[2, 2].set_ylim(0,350)
    axes[2, 2].set_title("Power Spectrum")
    axes[2, 2].set_xlabel("Normalized Spatial Frequency")
    # axes[2, 2].set_xlim(0,int(s[1]/2))
    axes[2, 2].legend()
    
    # 位相差分を表示（赤い部分を範囲外としてマスク）
    axes[3, 2].imshow(phase_diff, cmap="seismic")
    axes[3, 2].imshow(mask, cmap="Reds", alpha=0.5)  # 範囲外を赤く塗る
    axes[3, 2].set_title("Phase Spectrum Difference ($ \Delta \phi > |\pi/2|$ Highlighted)")
    axes[3, 2].axis("off")
