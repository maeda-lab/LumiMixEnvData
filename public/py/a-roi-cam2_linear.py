...
# CONFIG
# ======================
CSV_PATH = r"D:\vectionProject\public\camera2images\cam2_tree_bbox.csv"  #  你的bbox CSV

VIDEO_LINEAR = r"D:\vectionProject\public\camera2images\cam2_linear.mp4"  #  只分析 linear

OUT_DIR = r"D:\vectionProject\public\camera2images\track_contrast_out_linear_only"
...
def analyze_linear_video(video_path, df_bbox):
    ...
    dbg_writer = None
    if MAKE_DEBUG_VIDEO:
        dbg_path = os.path.join(OUT_DIR, "linear_ROIbox_track.mp4")
    ...
    for i in range(n_frames):
        ...
        if dbg_writer is not None:
            ...
            txt = f"linear t={t_sec:.2f}s idx={t_idx:.2f} k={k} u={u:.2f}"
...
def plot_metric(df_linear, metric):
    plt.figure()
    plt.plot(df_linear["time_sec"], df_linear[metric], label="linear")
    plt.xlabel("time (sec)")
    plt.ylabel(metric)
    plt.title(f"Tracking ROI metric: {metric} (interp={INTERP_MODE})")
    plt.legend()
    plt.grid(True)
    out_png = os.path.join(OUT_DIR, f"linear_{metric}.png")
...
def main():
    ...
    out = analyze_linear_video(VIDEO_LINEAR, df)

    out_csv = os.path.join(OUT_DIR, "track_metrics_linear.csv")
    out.to_csv(out_csv, index=False)
    print("Saved:", out_csv)

    for metric in ["lap_var", "rms_contrast"]:
        plot_metric(out, metric)
