...
vid_beta = out_dir / "cross_dissolve_1d_sequence_ABCD_beta05_vertical.mp4"
...
def build_sequence_video(
    *,
    ...
    for frame_idx in range(n_frames):
        ...
        if mode == "beta":
            frame = add_label(frame, f"BOT beta-mix({beta:0.2f})  w={w_use:0.2f}", (10, H + 10))
        else:
            Aamp = amplitude_of_mix(w_use, d_step)
            gain = min(1.0 / max(amp_eps, Aamp), gain_cap)
            frame = add_label(frame, f"BOT phase+ampnorm  w={w_use:0.2f}  A={Aamp:0.2f}  gain={gain:0.2f}", (10, H + 10))
        frame = add_label(frame, f"cycles={cycles}  d={d_step/math.pi:0.2f}π  1s/transition  total={total_seconds:.0f}s", (10, 2*H - 18))
...
# Generate beta mix (recommended first)
frames_beta, fps = build_sequence_video(mode="beta", beta=0.5)
write_mp4(vid_beta, frames_beta, fps)

# Generate amplitude-normalized (to reduce "all blurry then pop")
frames_ampn, fps = build_sequence_video(mode="ampnorm", amp_eps=0.08, gain_cap=2.5)
