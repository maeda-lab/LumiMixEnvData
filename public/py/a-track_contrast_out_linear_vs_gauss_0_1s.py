...
OUT_DIR = r"D:\vectionProject\public\camera2images\track_contrast_out_linear_vs_gauss_0_1s"
...
def analyze_one_video(name, video_path, df_bbox):
    ...
    try:
        ...
        fps_real = float(meta.get("fps", FPS))
        if not np.isfinite(fps_real) or fps_real <= 0:
            fps_real = float(FPS)
    except Exception:
        fps_real = float(FPS)

    n_limit = int(round(fps_real * TIME_LIMIT_SEC))
    ...
    dbg_writer = None
    if MAKE_DEBUG_VIDEO:
        dbg_path = os.path.join(OUT_DIR, f"{name}_ROIbox_track_0_1s.mp4")
        dbg_writer = imageio.get_writer(dbg_path, fps=fps_real, codec="libx264", quality=9)
    ...
    for i in range(n_limit):
        ...
        # 注意：时间轴仍按“视频真实fps”推进，t_idx 按你的 SECONDS_PER_STEP 计算
        t_sec = i / fps_real
...
def plot_metric_0_1s(combo, metric, out_name):
    plt.figure()
    ...
    plt.xlabel("time (sec)")
    plt.ylabel(metric)
    plt.title(f"{metric} (0–{TIME_LIMIT_SEC:.0f}s, interp={INTERP_MODE})")
    plt.legend()
    plt.grid(True)
...
def main():
    ...
    df_lin = analyze_one_video("linear", VIDEO_LINEAR, df)
    df_lin.to_csv(os.path.join(OUT_DIR, "track_metrics_linear_0_1s.csv"), index=False)
    results.append(df_lin)

    print("Analyzing: gauss", VIDEO_GAUSS)
    df_gau = analyze_one_video("gauss", VIDEO_GAUSS, df)
    df_gau.to_csv(os.path.join(OUT_DIR, "track_metrics_gauss_0_1s.csv"), index=False)
    results.append(df_gau)

    combo = pd.concat(results, ignore_index=True)
    combo.to_csv(os.path.join(OUT_DIR, "track_metrics_LINEAR_GAUSS_0_1s.csv"), index=False)

    # 你要的两张图（0–1s）
    plot_metric_0_1s(combo, "lap_var", "lap_var_0_1s_linear_vs_gauss.png")
    plot_metric_0_1s(combo, "rms_contrast", "rms_contrast_0_1s_linear_vs_gauss.png")
